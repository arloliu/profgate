...
// PROFGATE_E2E_PKCE_VERIFIER_OVERRIDE when that variable is set and non-empty, and the verifier Authorize generated otherwise.
// The end-to-end lanes set the variable to a verifier that does not match
// the challenge the device request carried, so an issuer that enforces PKCE refuses the login.
