...
func TestDispatchGrammar(t *testing.T) {
	tests := []struct {
		...
		{name: "-n after the positionals", args: []string{"smoke", "x/y", "cpu", "-n", "payments"}, wantCode: 2, wantStderr: "profgate: -n is not a flag; the namespace flag is --namespace\n"},
		{name: "-n before the verb", args: []string{"-n", "payments", "smoke", "x/y", "cpu"}, wantCode: 2, wantStderr: "profgate: -n is not a flag; the namespace flag is --namespace\n"},
		{name: "-o where the command line registers none", args: []string{"smoke", "x/y", "cpu", "-o", "json"}, wantCode: 2, wantStderr: "profgate: -o is not a flag; the output format flag is --output\n"},
