...
func scenarioAuthBasic(t *testing.T, h *Harness) {
	...
	if limits.CPUSeconds != 60 || limits.TraceSeconds != 60 || string(limits.Pprof.Default) != `{"port":6060}` ||
		len(limits.Pprof.AllowedSelections) != 0 || limits.Pprof.AllowedSelections == nil || limits.PGO.Enabled {
		t.Fatalf("limits: %s, want cpuSeconds 60, traceSeconds 60, pprof.default {\"port\":6060}, an empty allowedSelections, and pgo.enabled false", body)
	}
}
